<template>
  <div style="height:100%;">
    ...
    <div class="state-table-main">
      <main-table :data="rowData" :total="total" :rowProps="{prop: 'cpuModel', label: 'CPU型号'}"></main-table>
    </div>
  </div>
</template>
...
import { getCpuModelStatus } from '@/common/api/wholeStatus';
import StateCount from '../../state-count';
import MainTable from './main-table';
export default {
  name: 'stateWholeCpuModel',
  ...
  setup (props) {
    ...
    const queryList = async () => {
      let res = await getCpuModelStatus(props.versionName);
      if (res.code === 200) {
