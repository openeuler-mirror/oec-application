<template>
  <div style="height:100%;">
    ...
    <div class="state-table-main">
      <main-table :data="rowData" :total="total" :rowProps="{prop: 'cpuFactory', label: 'CPU厂商'}"></main-table>
    </div>
  </div>
</template>
...
import { getCpuFactoryStatus } from '@/common/api/wholeStatus';
import StateCount from '../../state-count';
import MainTable from './main-table';
export default {
  name: 'stateWholeCpuFactory',
  ...
  setup (props) {
    ...
    const queryList = async () => {
      let res = await getCpuFactoryStatus(props.versionName);
      if (res.code === 200) {
