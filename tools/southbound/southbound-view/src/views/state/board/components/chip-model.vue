<template>
  <div style="height: 100%;">
    ...
    <div class="state-table-main">
      <main-table :data="rowData" :total="total" :rowProps="{prop: 'chipModel', label: '芯片型号'}"></main-table>
    </div>
  </div>
</template>
...
import { getChipModelStatus } from '@/common/api/boardStatus';
import StateCount from '../../state-count';
import MainTable from './main-table';

export default {
  name: 'stateBoardChipModel',
  ...
  setup (props) {
    ...
    const queryList = async () => {
      let res = await getChipModelStatus(props.versionName);
      if (res.code === 200) {
