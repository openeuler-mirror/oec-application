  ...
  <div class="login-container">
    <div class="login-icon-group">
      <div class="login-icon-group-title">
        <img class="pointer" src="@/assets/icon.svg" @click="open" />
      </div>
      <img src="@/assets/compatibility.png" style="width: 498px; height: 464px; margin-left: 50px" class="login-icon-group-icon"/>
    </div>
    <div class="login-content">
      <div class="login-content-main">
        <h4 class="login-content-title" :style="{ opacity: loadFinish ? 1 : 0 }">
          {{ !isFirstLogin ? "南向看板系统" : "设置超级管理员密码" }}
        </h4>
        <account-login v-if="!isFirstLogin && loadFinish"></account-login>
        <first-login v-if="isFirstLogin && loadFinish" @setSuccess="setSuccess"></first-login>
      </div>
    </div>
  </div>
</template>
...
<script>
import { ref } from 'vue';
import { getAdminStatus } from '@/common/api/login';
import AccountLogin from './account-login';
import FirstLogin from './first-login';

export default {
  name: 'loginIndex',
  components: { AccountLogin, FirstLogin },
  setup() {
    let isFirstLogin = ref(false);
    let loadFinish = ref(false);
    const getStatus = async () => {
      let res = await getAdminStatus();
      if (res.code === 200) {
        isFirstLogin.value = res.data.isFirstLogin;
    ...
    const open = () => {
      const url = 'https://www.openeuler.org/zh/compatibility/';
      window.open(url);
    };
