<template>
  <el-dialog v-model="dialogVisible" :before-close="closeHandle" :close-on-click-modal="false" draggable :title="type === 'add' ? '新增板卡计划' : '修改板卡计划'" width="40vw">
    <el-form v-if="dialogVisible" ref="formRef" :model="formData" :rules="rules" label-width="145px" status-icon>
      ...
      <el-form-item label="版本发布前适配型号" prop="betaList">
        <el-select v-model="formData.betaList" multiple collapse-tags collapse-tags-tooltip style="width: 50%;" @change="handleListChange($event, 'beta')" placeholder="请选择适配型号" clearable>
      ...
      <el-form-item label="版本发布后适配型号" prop="releaseList">
        <el-select v-model="formData.releaseList" multiple collapse-tags collapse-tags-tooltip style="width: 50%;" @change="handleListChange($event, 'release')" placeholder="请选择适配型号" clearable>
